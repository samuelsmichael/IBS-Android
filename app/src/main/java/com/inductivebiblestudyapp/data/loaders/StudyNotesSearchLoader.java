...
/**
 * The loader responsible for study note searches.
 * @author Jason Jenkins
 * @version 0.1.0-20150815
 */
public class StudyNotesSearchLoader extends AbstractFetchAsyncLoader<StudyNotesResponse> {
	final static private String CLASS_NAME = StudyNotesSearchLoader.class
			.getSimpleName();
	...
	public StudyNotesSearchLoader(Context context, Bundle args, OnWebRequestListener listener) {
		super(context);
		testArgs(args);
	...
	@Override
	protected StudyNotesResponse fetchResult() {
		/*
		 * Remember: 
		...
		StudyNotesResponse result = AppCache.getStudyNotesSearchResponse(key);
		if (result == null) {
			mWebListener.onWebRequest();
			result = mService.searchStudyNotes(mAccessToken, mQuery, bookId, chapterId, verseStartId, verseEndId);
			AppCache.addStudyNotesSearchResponse(key, result);
		}
		return result;
	}
